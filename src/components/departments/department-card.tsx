...
export function DepartmentCard({ dept, isAdmin, canLockUnlock, onEdit, onLock, onUnlock, onDelete, onMobileMenu }: DepartmentCardProps) {
        ...
        {isAdmin ? (
          <span data-menu>
            <button
              ...
              onClick={(e) => { e.stopPropagation(); dept.isScheduleLocked ? onUnlock(dept) : onLock(dept); }}
            >
              {dept.isScheduleLocked ? <Lock className="h-4 w-4 text-amber-500" /> : <Unlock className="h-4 w-4 text-gray-400" />}
            </button>
          </span>
        ) : dept.isScheduleLocked ? (
          <span title="Schedule locked"><Lock className="h-4 w-4 text-amber-500" /></span>
        ) : null}
      </div>
          ...
          <div className="hidden md:block">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {isAdmin && <DropdownMenuItem onClick={() => onEdit(dept)}>Edit Department</DropdownMenuItem>}
                {canLockUnlock && (
                  <>
                    <DropdownMenuSeparator />
                    {!dept.isScheduleLocked && <DropdownMenuItem onClick={() => onLock(dept)}>Lock Schedule</DropdownMenuItem>}
                    {dept.isScheduleLocked && <DropdownMenuItem onClick={() => onUnlock(dept)}>Unlock Schedule</DropdownMenuItem>}
                  </>
                )}
                {isAdmin && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem className="text-red-600" onClick={() => onDelete(dept)}>Delete Department</DropdownMenuItem>
                  </>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          <Button size="icon" variant="ghost" className="md:hidden h-11 w-11 touch-manipulation" onClick={(e) => { e.stopPropagation(); onMobileMenu(dept); }} aria-label="Actions">
