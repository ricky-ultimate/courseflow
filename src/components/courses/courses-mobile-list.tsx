...
export function CoursesMobileList({ courses, canEditCourse, isAdmin, onView, onDelete }: CoursesMobileListProps) {
      ...
      <div className="md:hidden space-y-3">
        {courses.map((c) => (
          <div
            ...
            onClick={(e) => { if (!(e.target as HTMLElement).closest("[data-menu]")) onView(c); }}
          >
            ...
            <div className="mt-3 pt-3 border-t flex justify-between items-center">
              ...
              <div data-menu>
                <Button size="icon" variant="ghost" className="h-11 w-11 touch-manipulation" onClick={(e) => { e.stopPropagation(); setMenuCourse(c); }}>
        ...
        <SheetContent side="bottom" className="rounded-t-2xl">
          ...
          <div className="flex flex-col gap-1 py-4">
            {menuCourse && (() => {
              ...
              return (
                <>
                  <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { onView(c); close(); }}>View Details</button>
                  {canEditCourse(c) && (
                    <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { router.push(`/courses/${c.code}/edit`); close(); }}>Edit Course</button>
                  )}
                  {isAdmin && (
                    <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-red-50 text-red-600 text-left w-full min-h-[52px] font-medium" onClick={() => { onDelete(c); close(); }}>Delete Course</button>
                  )}
                </>
              );
            })()}
