...
export function CoursesTable({ courses, canEditCourse, isAdmin, onView, onDelete }: CoursesTableProps) {
              ...
              <tr key={c.id} className="border-t hover:bg-gray-50">
                ...
                <td className="p-3 text-right">
                  <div className="flex items-center justify-end gap-1 min-w-0">
                    <Button size="icon" variant="ghost" className="h-11 w-11 touch-manipulation" onClick={() => onView(c)}>
                      <Eye className="h-5 w-5" /><span className="sr-only">View</span>
                    </Button>
                    {canEditCourse(c) && (
                      <Button size="icon" variant="ghost" className="h-11 w-11 touch-manipulation" onClick={() => router.push(`/courses/${c.code}/edit`)}>
                        <Pencil className="h-5 w-5" /><span className="sr-only">Edit</span>
                      </Button>
                    )}
                    {isAdmin && (
                      <Button size="icon" variant="ghost" className="h-11 w-11 text-red-600 touch-manipulation" onClick={() => onDelete(c)}>
