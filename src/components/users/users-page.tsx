...
function getUserAvatarColor(name: string): string {
  ...
  return USER_LIST_AVATAR_COLORS[
    Math.abs(hash) % USER_LIST_AVATAR_COLORS.length
  ]!;
}
...
export function UsersPage({ role }: UsersPageProps) {
  ...
  const openEdit = async (u: UserType) => {
    ...
    try {
      ...
      if (openForEditUserIdRef.current === u.id)
  ...
  const handleToggleActive = async (u: UserType): Promise<void> => {
    ...
    try {
      ...
      const res = await apiClient.updateUser(u.id, {
  ...
  const handleConfirmDeactivate = async (): Promise<boolean> => {
    ...
    try {
      ...
      const res = await apiClient.updateUser(deactivateUser.id, {
        name: deactivateUser.name ?? undefined,
      ...
      >
        {showDeptSelect && (
          <FilterSelect
            ...
            onValueChange={(v) => setDepartmentCode(v === "all" ? "" : v)}
            width="w-[180px]"
          >
