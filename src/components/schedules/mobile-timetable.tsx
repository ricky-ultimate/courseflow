...
import { WEEKDAYS, DAY_MEDIUM_LABELS, TIME_SLOTS, getDeptStyle } from "@/lib/constants";
...
export function MobileTimetable({
  schedules, selectedDay, onDayChange, onScheduleClick, onEmptySlotClick, canMutate,
  ...
  useEffect(() => {
    ...
    if (!el) return;
    el.querySelector("[data-active='true']")?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
  }, [selectedDay]);
        ...
        {WEEKDAYS.map((day) => (
          <button
            ...
            className={`shrink-0 h-10 rounded-full px-5 font-semibold text-sm tracking-wide touch-manipulation transition-all ${
              day === selectedDay ? "bg-indigo-600 text-white shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
            ...
            return (
              <div key={startTime} className="flex gap-4 min-h-[96px] relative group">
                ...
                >
                  ...
                  <div className="text-[11px] font-medium text-slate-500 mt-2.5">{s.startTime} – {s.endTime}</div>
                </button>
              </div>
            );
          }
