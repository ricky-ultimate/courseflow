...
export function TimetableGrid({
  ...
  const getSchedule = (day: DayOfWeek, startTime: string) =>
    scheduleMap.get(`${day}-${startTime}`);

  const isCoveredByEarlierSchedule = (
    day: DayOfWeek,
    slotTime: string,
  ): boolean => {
    const slotHour = getStartHour(slotTime);
    for (const [key, s] of Array.from(scheduleMap.entries())) {
      if (!key.startsWith(day)) continue;
      const sStartHour = getStartHour(s.startTime);
  ...
  return (
    ...
    >
      ...
      {WEEKDAYS.map((day, i) => {
        ...
        const dateNum =
          today.getDate() - monOffset + ((DAY_TO_JS[day] ?? 1) - 1);
        return (
          <div
            key={day}
          ...
          >
            ...
            <span
              className={`flex h-6 w-6 items-center justify-center rounded-full text-sm font-semibold ${isToday ? "bg-indigo-600 text-white" : "text-slate-800"}`}
            >
              {dateNum}
            </span>
          </div>
      ...
      {schedules
        .filter((s) =>
          WEEKDAYS.includes(s.dayOfWeek as (typeof WEEKDAYS)[number]),
        )
        .map((s) => {
          ...
          if (!col || rowStart < 2 || rowStart > 11 || span < 1) return null;
          ...
          return (
            ...
            >
              ...
              <div
                className={`text-sm font-bold tracking-tight leading-tight truncate ${style.text}`}
              >
                {s.course?.code ?? s.courseCode}
              </div>
              <div className="text-xs text-slate-600/90 truncate leading-snug mt-0.5">
                {s.course?.name ?? "—"}
              </div>
      ...
      {TIME_SLOTS.map((startTime, rowIdx) =>
        WEEKDAYS.map((day, colIdx) => {
          ...
          const covered = isCoveredByEarlierSchedule(day, startTime);
          ...
          return (
            ...
            >
              {canMutate && !isCutoff && (
                ...
                >
                  <span className="text-transparent group-hover:text-indigo-400 text-lg font-light transition-colors">
                    +
                  </span>
                </button>
              )}
