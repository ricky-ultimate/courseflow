...
interface UnscheduledExamsPanelProps {
  onScheduleExam: (courseCode: string) => void;
}

const INITIAL_VISIBLE = 5;

export function UnscheduledExamsPanel({
  onScheduleExam,
}: UnscheduledExamsPanelProps) {
  ...
  const fetchUnscheduled = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.getCoursesWithoutExams();
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm font-medium text-amber-800">
            {courses.length} course{courses.length !== 1 ? "s" : ""} not yet
            scheduled for exams
          </span>
        </div>
        {panelOpen ? (
