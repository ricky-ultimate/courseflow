...
export default function DepartmentDetailsPage() {
    ...
    apiClient
      ...
      .then((response) => {
        if (response.success && response.data) {
          ...
          setCourses(
            Array.isArray(deptWithCourses.courses)
              ? deptWithCourses.courses
              : [],
          );
        } else {
  ...
  const fetchSchedules = useCallback(async () => {
    ...
    try {
      const res = await apiClient.getSchedules({ limit: 100 });
      const r = getItemsFromResponse<Schedule>(res);
      const all = r?.items ?? [];
      // Filter to schedules whose course belongs to this department
      const courseCodes = new Set(courses.map((c) => c.code));
      const filtered = all.filter(
        (s) =>
          s.course?.departmentCode === code || courseCodes.has(s.courseCode),
      );
  ...
  useEffect(() => {
    if (code && !loading) fetchSchedules();
  }, [code, loading, fetchSchedules]);
    ...
    <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {canAddCourse && (
                <Button
                  size="sm"
                  onClick={() =>
                    router.push(`/courses/create?department=${code}`)
                  }
                >
      ...
      <Dialog open={editOpen} onOpenChange={(o) => !o && setEditOpen(false)}>
        <DialogContent
          ...
          onSwipeDown={() => setEditOpen(false)}
        >
          <DialogHeader>
            <DialogTitle>Edit Department</DialogTitle>
            <DialogDescription>
              Update department details and assign HOD.
            </DialogDescription>
          </DialogHeader>
          <Form {...editForm}>
            <form
              onSubmit={editForm.handleSubmit(async (data) => {
                ...
                try {
                  setEditSaving(true);
                  const res = await apiClient.updateDepartment(
                    department.code,
                    {
                      name: data.name,
                      code: data.code,
