...
import { Building2, Download, Upload, Plus, Filter, Lock, Unlock, Trash2 } from "lucide-react";
...
export default function DepartmentsPage() {
  ...
  const fetchDepartments = useCallback(async () => {
    try {
      ...
      if (result) { setDepartments(result.items); setTotalPages(result.totalPages); setTotal(result.total); }
    } catch {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 mt-1">Manage and browse all department listings</p>
        </div>
        <div className="flex items-center gap-2">
          {isAdmin && (
            <>
              ...
              <Button size="sm" className="rounded-full" onClick={() => router.push("/departments/create")}>
      ...
      <FilterBar searchValue={searchTerm} onSearchChange={setSearchTerm} searchPlaceholder="Search by name or code...">
        <div className="hidden md:flex items-center gap-1">
          <button type="button" onClick={() => setHasCourses(!hasCourses)}
          ...
          <button type="button" onClick={() => setWithoutCourses(!withoutCourses)}
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4">
          ...
          {departments.map((dept) => (
            <DepartmentCard
              ...
              onLock={(d) => setConfirmAction({ open: true, type: "lock", dept: d })}
              onUnlock={(d) => setConfirmAction({ open: true, type: "unlock", dept: d })}
              onDelete={(d) => setConfirmAction({ open: true, type: "delete", dept: d })}
      ...
      <Sheet open={!!mobileDeptMenu} onOpenChange={(o) => !o && setMobileDeptMenu(null)}>
        <SheetContent side="bottom" className="rounded-t-2xl">
          <SheetHeader><SheetTitle>{mobileDeptMenu?.name ?? "Department actions"}</SheetTitle></SheetHeader>
          <div className="flex flex-col gap-1 py-4">
            {mobileDeptMenu && (() => {
              ...
              return (
                <>
                  <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { router.push(`/departments/${d.code}`); close(); }}>View Details</button>
                  {isAdmin && <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { setEditDept(d); close(); }}>Edit Department</button>}
                  {!d.isScheduleLocked && isAdmin && <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { setConfirmAction({ open: true, type: "lock", dept: d }); close(); }}>Lock Schedule</button>}
                  {d.isScheduleLocked && isAdmin && <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 text-left w-full min-h-[52px] font-medium" onClick={() => { setConfirmAction({ open: true, type: "unlock", dept: d }); close(); }}>Unlock Schedule</button>}
                  {isAdmin && <button type="button" className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-red-50 text-red-600 text-left w-full min-h-[52px] font-medium" onClick={() => { setConfirmAction({ open: true, type: "delete", dept: d }); close(); }}>Delete Department</button>}
                </>
              );
            })()}
          </div>
      ...
      <DepartmentUploadModal open={isUploadOpen} onOpenChange={setIsUploadOpen} onSuccess={fetchDepartments} />
      <DepartmentEditModal dept={editDept} onClose={() => setEditDept(null)} onSuccess={fetchDepartments} />
