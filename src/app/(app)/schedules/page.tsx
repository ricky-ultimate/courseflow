...
export default function SchedulePage() {
  ...
  const deptCodeToName = Object.fromEntries(
  ...
  useEffect(() => {
    if (!defaultFiltersApplied.current && sessions.length > 0 && !loading) {
      ...
      if (cscDept) {
        setSelectedDepartment(cscDept.code);
  ...
  const getTimeSlots = (scheds: Schedule[]): string[] => {
    ...
    scheds.forEach((schedule) =>
      timeSet.add(`${schedule.startTime}-${schedule.endTime}`),
    );
  ...
  const formatTimeSlot = (startTime: string, endTime: string): string => {
    try {
      ...
      if (startAMPM === endAMPM)
        return `${dispStart}-${dispEnd} (${startAMPM})`;
  ...
  const buildExportData = (allSchedules: Schedule[]) => {
    ...
    allSchedules.forEach((s) => {
      ...
      if (!courseMap.has(s.course.code)) {
        courseMap.set(s.course.code, {
          ...
          title: s.course.name,
          lecturer:
            s.course.lecturer?.name || s.course.lecturer?.email || "N/A",
    ...
    const timetableRows = dayOptions.map((day) => {
      ...
      timeSlots.forEach((ts) => {
        ...
        row.push(
          scheds.length > 0 ? scheds.map((x) => x.code).join(" / ") : "",
        );
      });
      return row;
    });
                ...
                if (schedule.course && enrichedCourse) {
                  return {
                    ...
                    course: {
                      ...schedule.course,
                      lecturer:
                        enrichedCourse.lecturer || schedule.course.lecturer,
                    },
                  } as Schedule;
                }
              }
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  <DropdownMenuItem
                    ...
                    className="cursor-pointer py-2"
                  >
                    <FileText className="h-4 w-4 mr-2" /> Image (PNG)
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
          ...
          {filteredSchedules.length === 0 ? (
            <div className="py-24 text-center flex flex-col items-center">
              ...
              <p className="text-slate-500 mt-2 max-w-md">
                There are no classes scheduled for the selected parameters.
                Adjust your filters or generate a new timetable.
              </p>
            </div>
          ) : viewMode === "agenda" ? (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-3 pl-2">
                            ...
                            <div className="flex gap-1">
                              ...
                              {s.isManualOverride && (
                                <div
                                  className="h-2 w-2 rounded-full bg-amber-400 mt-1"
                                  title="Manual"
                                />
                              )}
                            </div>
