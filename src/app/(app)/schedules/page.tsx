...
import { Calendar, Search, Clock, RefreshCw, FileText, Plus, FileDown, ChevronDown, Trash2 } from "lucide-react";
...
import { exportAsPDF, exportAsXLSX, exportAsCSV, exportAsPNG } from "@/lib/schedule-export";
...
export default function SchedulePage() {
  ...
  useEffect(() => {
    if (!defaultFiltersApplied.current && sessions.length > 0 && !loading) {
      const cscDept = departments.find((d) => d.code === "CS" || d.code === "CSC" || d.name.toLowerCase().includes("computer"));
      if (cscDept) setSelectedDepartment(cscDept.code);
      else if (departments.length > 0) setSelectedDepartment(departments[0]!.code);
  ...
  const fetchSchedules = useCallback(async () => {
    try {
      ...
      if (result) { setSchedules(result.items); setTotalPages(result.totalPages); setTotal(result.total); }
    } catch (error) {
          ...
          if (courses.length > 0) {
            ...
            allSchedules = allSchedules.map((schedule) => {
              if (schedule.courseCode && courseMap.has(schedule.courseCode)) {
                ...
                if (schedule.course) return { ...schedule, course: { ...schedule.course, lecturer: enriched.lecturer ?? schedule.course.lecturer } } as Schedule;
              }
              return schedule;
            });
          }
  ...
  const runExport = async (fn: (s: Schedule[]) => Promise<void>, label: string) => {
    try {
      toast({ title: "Preparing Export", description: "Fetching all schedules..." });
      const allSchedules = await fetchAllSchedulesForExport();
      if (!allSchedules.length) { toast({ title: "No Schedules", description: "There are no schedules to export", variant: "destructive" }); return; }
      await fn(allSchedules);
      toast({ title: "Export Successful", description: `Timetable exported as ${label} (${allSchedules.length} schedules)` });
    } catch {
      toast({ title: "Export Error", description: `An error occurred while exporting ${label}`, variant: "destructive" });
    }
  };
  ...
  const groupedSchedules = WEEKDAYS.map((day) => ({
    day,
    schedules: filteredSchedules.filter((s) => s.dayOfWeek === day).sort((a, b) => a.startTime.localeCompare(b.startTime)),
    ...
    <div className="flex flex-col gap-6 md:gap-8 pb-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex bg-slate-100/80 p-1 rounded-xl shadow-inner border border-slate-200/50 mr-2">
            <button onClick={() => setViewMode("agenda")} className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-all ${viewMode === "agenda" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-900"}`}>Agenda</button>
            <button onClick={() => setViewMode("timetable")} className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-all ${viewMode === "timetable" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-900"}`}>Grid</button>
          </div>
          {canMutateSchedules && (
            <>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 p-1.5 bg-white border border-slate-200 rounded-2xl shadow-sm w-full">
        <div className="relative flex-1 w-full">
          ...
          <Input placeholder="Search courses..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="border-0 bg-transparent shadow-none focus-visible:ring-0 pl-9 h-10 w-full" />
        </div>
        ...
        <div className="flex items-center gap-1 w-full sm:w-auto overflow-x-auto no-scrollbar px-1">
          ...
          {filterCount > 0 && <button onClick={handleReset} className="text-xs text-slate-500 hover:text-slate-800 ml-2 px-2 whitespace-nowrap">Clear</button>}
        </div>
      </div>
      ...
      {fetchError ? (
        ...
        <div className="relative">
          ...
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm font-medium text-slate-500">Showing {filteredSchedules.length} classes</p>
            {filteredSchedules.length > 0 && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="rounded-full shadow-sm">
                    <FileDown className="h-4 w-4 mr-2 text-slate-500" />Export<ChevronDown className="h-3 w-3 ml-2 text-slate-400" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-lg">
                  <DropdownMenuItem onClick={() => runExport(exportAsPDF, "PDF")} className="cursor-pointer py-2"><FileText className="h-4 w-4 mr-2" />PDF Document</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => runExport(exportAsXLSX, "XLSX")} className="cursor-pointer py-2"><FileText className="h-4 w-4 mr-2" />Excel (XLSX)</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => runExport(exportAsCSV, "CSV")} className="cursor-pointer py-2"><FileText className="h-4 w-4 mr-2" />CSV File</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => runExport(exportAsPNG, "PNG")} className="cursor-pointer py-2"><FileText className="h-4 w-4 mr-2" />Image (PNG)</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
          </div>
          {filteredSchedules.length === 0 ? (
            <div className="py-24 text-center flex flex-col items-center">
              ...
              <p className="text-slate-500 mt-2 max-w-md">No classes scheduled for the selected parameters. Adjust your filters or generate a new timetable.</p>
            </div>
          ) : viewMode === "agenda" ? (
              ...
              {groupedSchedules.map(({ day, schedules: daySchedules }) => (
                <div key={day} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {daySchedules.map((s) => (
                      <div key={s.id} onClick={() => openScheduleDetail(s)} className="group bg-white border border-slate-200 rounded-2xl p-5 hover:shadow-md hover:border-slate-300 transition-all cursor-pointer relative overflow-hidden">
                        <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-indigo-500 rounded-l-2xl opacity-80" />
                        <div className="flex justify-between items-start mb-3 pl-2">
                          <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-700">{s.startTime} – {s.endTime}</span>
                          {s.isManualOverride && <div className="h-2 w-2 rounded-full bg-amber-400 mt-1" />}
                        </div>
                        <div className="pl-2">
                          ...
                          <div className="flex items-center gap-3 text-xs font-medium text-slate-400">
                            ...
                            <span>{s.course?.level?.replace("LEVEL_", "") ?? "—"} Lvl</span>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <>
              <div className="hidden md:block">
                <TimetableGrid schedules={filteredSchedules} onScheduleClick={openScheduleDetail} onEmptyCellClick={canMutateSchedules ? (day, startTime) => { setCreateModalPrefill({ dayOfWeek: day, startTime }); setEditSchedule(null); setCreateModalOpen(true); } : undefined} canMutate={canMutateSchedules} />
              </div>
              <div className="md:hidden">
                <MobileTimetable schedules={filteredSchedules} selectedDay={mobileSelectedDay} onDayChange={setMobileSelectedDay} onScheduleClick={openScheduleDetail} onEmptySlotClick={canMutateSchedules ? (day, startTime) => { setCreateModalPrefill({ dayOfWeek: day, startTime }); setEditSchedule(null); setCreateModalOpen(true); } : undefined} canMutate={canMutateSchedules} />
              </div>
            </>
          )}
          {total > 0 && (
            <div className="mt-8">
              <Pagination page={currentPage} totalPages={Math.max(1, totalPages)} total={total} limit={limit} onPageChange={setCurrentPage} onLimitChange={(v) => { setLimit(v); setCurrentPage(1); }} resultsLabel="schedules" />
            </div>
          )}
        </div>
      )}
