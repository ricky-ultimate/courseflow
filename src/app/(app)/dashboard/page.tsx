...
function MobileTimetable({ schedules }: { schedules: Schedule[] }) {
  ...
  const getDeptColor = (code: string) =>
    DEPT_COLOR_CLASSES[
      Math.abs(
        code.split("").reduce((a, c) => a + c.charCodeAt(0), 0)
      ) % DEPT_COLOR_CLASSES.length
    ];
      ...
      <div className="mt-4 space-y-3">
        {daySchedules.length === 0 ? (
          ...
          daySchedules.map((s) => (
            <div
              key={s.id}
              className={`rounded-lg p-3 min-h-[72px] ${getDeptColor(
                s.course?.departmentCode ?? "X"
              )}`}
            >
              ...
              {totalSchedules > 0 && (
                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-gray-500">
                      {manualOverrides} manual override{manualOverrides !== 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
              )}
          ...
          <ConfirmDialog
            ...
            description={
              department?.isScheduleLocked
                ...
                : `This will lock the schedule for ${
                    department?.name ?? "this department"
                  }. No further changes can be made until it is unlocked.`
            }
