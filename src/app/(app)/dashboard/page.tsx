...
function MobileTimetable({ schedules }: { schedules: Schedule[] }) {
  ...
  const getDeptColor = (code: string) =>
    DEPT_COLOR_CLASSES[
      Math.abs(code.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) %
      ...
      <div className="mt-4 space-y-3">
        {daySchedules.length === 0 ? (
          ...
          daySchedules.map((s) => (
            <div
              key={s.id}
              className={`rounded-lg p-3 min-h-[72px] ${getDeptColor(s.course?.departmentCode ?? "X")}`}
            >
      ...
      {isAdmin && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <StatCard
              icon={Clock}
              value={scheduleStats?.totalSchedules ?? 0}
                  ...
                  {deptBreakdown.map(([code, count], idx) => {
                    ...
                    return (
                      <div key={code} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-6 bg-gray-100 rounded overflow-hidden">
                          <div
                            className={`h-full rounded transition-all ${DEPT_BAR_COLORS[idx % DEPT_BAR_COLORS.length]}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
