...
import { AcademicSession, ApiResponse, SessionStatistics } from "@/types";
...
export default function AcademicSessionsPage() {
  ...
  const openEditSession = useCallback(
    async (s: AcademicSession) => {
      ...
      try {
        ...
        if (openForEditRef.current === s.id)
  ...
  const handleConfirmAction = async (): Promise<boolean> => {
    ...
    try {
      ...
      let res: ApiResponse<AcademicSession>;
      if (action === "activate")
  ...
  return (
    <div className="space-y-6">
          ...
          {sessions.map((session) => {
            ...
            return (
              <SessionCard
                ...
                onArchive={(s) =>
                  setConfirmState({ open: true, action: "archive", session: s })
                }
                onEdit={openEditSession}
                onStats={openStatsModal}
                onDelete={(s) =>
                  setConfirmState({ open: true, action: "delete", session: s })
                }
      ...
      <SessionFormModal
        ...
        description="Define the academic year and its start/end dates."
        form={createForm}
      ...
      <SessionFormModal
        ...
        title="Edit Session"
        description="Update the academic session details."
        form={editForm}
        onSubmit={handleEdit}
        submitting={submitting}
        error={editError}
