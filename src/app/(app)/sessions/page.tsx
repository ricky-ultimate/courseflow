...
export default function AcademicSessionsPage() {
  ...
  const [statsSession, setStatsSession] = useState<AcademicSession | null>(
    null,
  );
  ...
  const [mobileSheetSession, setMobileSheetSession] =
        ...
        items.map(async (s) => {
          try {
            ...
            if (res.success && res.data)
              stats[s.id] = res.data as SessionStatistics;
          } catch {
    ...
    async (s: AcademicSession) => {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          editForm.reset({
            name: fresh.name,
            startDate: fresh.startDate.split("T")[0],
            endDate: fresh.endDate.split("T")[0],
          });
        }
      } catch {
        if (openForEditSessionIdRef.current === s.id)
  ...
  const handleConfirmAction = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (action === "activate")
        res = await apiClient.activateAcademicSession(session.id);
      else if (action === "archive")
        res = await apiClient.archiveAcademicSession(session.id);
      else res = await apiClient.deleteAcademicSession(session.id);

      if (res.success) {
        if (action === "activate")
          toast({ title: `${session.name} is now the active session.` });
        else if (action === "archive")
          toast({ title: "Session archived.", variant: "info" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button
          size="default"
          className="h-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full"
      ...
      {fetchError ? (
        ...
        <div className="space-y-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="flex flex-col md:flex-row md:items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-6 py-5 shadow-sm animate-pulse"
            >
        ...
        <div className="relative space-y-3">
          ...
          {sessions.map((session) => {
            const isActive =
              activeSession?.id === session.id || session.isActive;
                      ...
                      <div className="hidden md:flex items-center gap-2">
                        {!isActive && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-9 border-indigo-600 text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700"
                            onClick={() =>
                              setConfirmState({
                                open: true,
                                action: "activate",
                        ...
                        <Button
                          ...
                          onClick={() => openStatsModal(session)}
                        >
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          className="h-11 w-11 text-red-600 hover:text-red-700 touch-manipulation"
                          onClick={() =>
                            setConfirmState({
                              open: true,
                              action: "delete",
      ...
      <Sheet
        open={!!mobileSheetSession}
        onOpenChange={(o) => !o && setMobileSheetSession(null)}
      >
        <SheetContent side="bottom" className="rounded-t-2xl">
          <SheetHeader>
            <SheetTitle>
              {mobileSheetSession?.name ?? "Session actions"}
            </SheetTitle>
          </SheetHeader>
      ...
      >
        <DialogContent
          ...
          onSwipeDown={() => setIsCreateDialogOpen(false)}
        >
          <DialogHeader>
          ...
          <Form {...createForm}>
            <form
              ...
              className={`space-y-4 transition-opacity ${creating ? "opacity-60" : ""}`}
            >
      ...
      <Dialog
        open={!!statsSession}
        onOpenChange={(o) => !o && setStatsSession(null)}
      >
        <DialogContent
          className="md:max-w-[440px]"
          onSwipeDown={() => setStatsSession(null)}
        >
          <DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-lg border p-4">
                ...
                <p className="text-2xl font-bold">
                  {statsData.schedulesBySemester?.FIRST ?? 0}
                </p>
              </div>
              <div className="rounded-lg border p-4">
                <p className="text-sm text-gray-500">
                ...
                <p className="text-2xl font-bold">
                  {statsData.schedulesBySemester?.SECOND ?? 0}
                </p>
              </div>
              <div className="rounded-lg border p-4">
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) =>
          !o &&
          setConfirmState({ open: false, action: "activate", session: null })
        }
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) =>
          !o &&
          setConfirmState({ open: false, action: "archive", session: null })
        }
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) =>
          !o &&
          setConfirmState({ open: false, action: "delete", session: null })
        }
